...
def find_corner(c1, c2, c3, cube):
    ...
    for corner in corners:
        c = [
            cube[corner[0]][corner[1]],
            cube[corner[2]][corner[3]],
            cube[corner[4]][corner[5]],
        ]
        ...
        while location[2:4] != [cross_colours[colour], 7]:
            ...
            if location[0] == 0:
                ...
                if not_free:
                    ...
                    if abs(m) == 3:
                        semi_sol += orientation(
                            {1: ["u", "f", "u'"], -1: ["u'", "f", "u"]}[m / abs(m)],
                            num_to_colour[location[0]],
                            "g",
                        )
                    elif abs(m) == 1:
                        semi_sol += orientation(
                        ...
                        semi_sol += orientation(
                            ["u", "u", "f", "u", "u"], num_to_colour[location[0]], "g"
                        )
                else:
                ...
                if not_free:
                    ...
                    if abs(m) == 3:
                        semi_sol += orientation(
                            {
                                1: ["u", {3: "f'", 5: "f"}[location[1]], "u'"],
                                -1: ["u'", {3: "f'", 5: "f"}[location[1]], "u"],
                            }[m / abs(m)],
                            num_to_colour[location[2]],
                            "g",
                        )
                    elif abs(m) == 1:
                        semi_sol += orientation(
    ...
    for corner in corner_pos:
        ...
        while [location[0], location[2]] != [5, cross_colours[corner[0]]]:
            ...
            if location[0] == 5:
                ...
                semi_sol += orientation(
                    ["r'", "d'", "r", "d"], front_for_r[location[1]], "g"
                )
...
def second_layer(cube):
    ...
    for edge in second_edges:
        ...
        while [location[0], location[2]] != [
            colour_to_num[edge[0]],
            colour_to_num[edge[1]],
        ]:
            semi_sol = []
            on_top = 0
            for i in [0, 2]:
            ...
            if on_top:
                ...
                if l_or_r == 1 or l_or_r == -3:
                    semi_sol += orientation(
                        ["u", "r", "u'", "r'", "u'", "f'", "u", "f"], edge[0], "b"
                    )
                else:
                    semi_sol += orientation(
                        ["u'", "l'", "u", "l", "u", "f", "u'", "f'"], edge[0], "b"
                    )
            else:
                ...
                for upper_edge in upper_edges:
                    if (
                        cube[upper_edge[0]][upper_edge[1]] == "b"
                        or cube[upper_edge[2]][upper_edge[3]] == "b"
                    ):
    ...
    while len(blue_corners) != 4:
        ...
        if len(blue_corners) == 0:
            ...
            if cube[right_colour[corner_loc[i]]][2] == "b":
                semi_sol += orientation(
                    ["r", "u", "r'", "u", "r", "u", "u", "r'"],
                    num_to_colour[right_colour[corner_loc[i]]],
                    "b",
                )
            else:
        ...
        if loc == 5:
            ...
            if cube[check[loc]][1] == opp_colour[check[loc]]:
                semi_sol += orientation(
                    ...
                    num_to_colour[check[loc]],
                    "b",
                )
            else:
                semi_sol += orientation(
                    ["l", "l", "u'", "f'", "b", "l", "l", "f", "b'", "u'", "l", "l"],
                    opp_colour[check[loc]],
                    "b",
                )
        moves(cube, semi_sol)
        solution += semi_sol
    ...
    while i < len(sol) - 1:
        ...
        if i < len(sol) - 2:
            if sol[i] == sol[i + 1] and sol[i + 1] == sol[i + 2]:
                ...
                check = 1
        if i < len(sol) - 1:
            if sol[i][0] == sol[i + 1][0] and (
                (len(sol[i]) == 1 and len(sol[i + 1]) == 2)
                or (len(sol[i]) == 2 and len(sol[i + 1]) == 1)
            ):
                del sol[i]
                del sol[i]
                check = 1
...
if __name__ == "__main__":
    ...
    print()
    print(len(sol[1]), sol[1])
    print()
    print(len(sol[2]), sol[2])
